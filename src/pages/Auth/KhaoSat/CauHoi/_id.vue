              ...
              <v-row dense>
                ...
                <v-col cols="12" sm="4" md="4">
                  <v-select
                      ...
                      item-text="name"
                      item-value="code"
                      :items="[{ code: 0, name: 'Câu hỏi cấp trên' }, { code: 1, name: 'Câu hỏi' }, { code: 2, name: 'Câu trả lời' }, { code: 3, name: 'Câu đặc biệt' }]"
                  />
                </v-col>
                <v-col cols="12" sm="4" md="4">
                  <v-select
                ...
                <v-col cols="12" sm="4" md="4">
                  <v-select
                      ...
                      item-text="name"
                      item-value="code"
                      :disabled="(item.danhDauCau != 2 && item.danhDauCau != 3)"
                ...
                <v-col cols="12" sm="3" md="3" hidden>
                  <v-select
...
export default {
  ...
  methods: {
    ...
    async fnGetDetail() {
      await this.$axios.get('auth/khao-sat/cau-hoi/id/' + this.$route.params.id).then((res) => {
        this.item = khaoSatCauHoiModel.fromJson(res.data.data)
        this.categoryId = this.item.maDanhMuc
      })
    },
    async fnSubmit() {
      this.loading = true
